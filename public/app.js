...
function calData() {
  ...
  for(let d=1;d<=l.getDate();d++) {
    ...
    S.vacations.forEach(v=>{if(new Date(ds)>=new Date(v.startDate)&&new Date(ds)<=new Date(v.endDate)){const my=v.userId===S.user?.id;if(my?S.filters.myVac:S.filters.team[v.userId])ev.push({type:'vacation',label:v.userName,userId:v.userId,my});}});
    days.push({day:d,date:ds,events:ev,isBlackout:bl,blackoutReason:blReason});
  }
  return days;
}

// Get my total days this year
function getMyDaysThisYear() {
  const year = new Date().getFullYear();
  return S.vacations.filter(v => v.userId === S.user?.id && new Date(v.startDate).getFullYear() === year).reduce((sum,v) => sum + v.days, 0);
}
