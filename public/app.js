...
const bizDays = (s,e) => { let c=0,d=new Date(s); while(d<=new Date(e)){if(d.getDay()%6)c++;d.setDate(d.getDate()+1)} return c; };
const fmt = d => new Date(d).toLocaleDateString('en-CA');
const fmtNice = d => new Date(d).toLocaleDateString('en-US', {month:'short',day:'numeric',year:'numeric'});
const fmtTime = d => new Date(d).toLocaleString('en-US', {month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'});
const isBlackout = (d,b) => b.some(x=>new Date(d)>=new Date(x.startDate)&&new Date(d)<=new Date(x.endDate));
const getBlackoutReason = (d,b) => { const bl = b.find(x=>new Date(d)>=new Date(x.startDate)&&new Date(d)<=new Date(x.endDate)); return bl ? bl.reason : ''; };
const hasOverlap = (s,e,b) => { let d=new Date(s); while(d<=new Date(e)){if(isBlackout(d,b))return true;d.setDate(d.getDate()+1)} return false; };
...
const isPast = (s,e) => new Date(e) < new Date(today());
...
function calData() {
  ...
  for(let d=1;d<=l.getDate();d++) {
    ...
    S.vacations.filter(v=>v.status==='approved').forEach(v=>{if(new Date(ds)>=new Date(v.startDate)&&new Date(ds)<=new Date(v.endDate)){const my=v.userId===S.user?.id;if(my?S.filters.myVac:S.filters.team[v.userId])ev.push({type:'vacation',label:v.userName,userId:v.userId,my});}});
...
function getMyDaysThisYear() {
  ...
  return S.vacations.filter(v => v.userId === S.user?.id && v.status === 'approved' && new Date(v.startDate).getFullYear() === year).reduce((sum,v) => sum + v.days, 0);
}
...
const userHistoryModalHTML=()=>{
  ...
  if(!u) return '';
  const userVacs = S.vacations.filter(v=>v.userId===u.id).sort((a,b)=>new Date(b.startDate)-new Date(a.startDate));
  const upcoming = userVacs.filter(v=>new Date(v.endDate)>=new Date());
  const past = userVacs.filter(v=>new Date(v.endDate)<new Date());
